...
;;Option for moving
;;shelf_kitchen_right:shelf:shelf_base_center
;;shelf_kitchen_left:shelf:shelf_base_center
;;table_kitchen_shelves:table:table_front_edge_center


;;Bei Pose:
;;Für Tisch fenstersite = +y
;;Für Tisch Computerseite = -y
;;Für Tisch in Richtung living room +X gegenteil dann -x

;;Für regale ist immer -x und dann den link bei object-ident austauschen




(defun src-gs ()
  ;;(move-hsr (create-pose (call-knowledge-object-rel-pose "hsr_pose_dinner_table")))
  ;;(su-real::with-hsr-process-modules
    (let*
        ...
        ((object-ident (call-knowledge2 "has_urdf_name" :param-list "table_kitchen_shelves:table:table_front_edge_center" :result 'object))
         ...
         ;; (?place-pose (call-knowledge-object-place-pose object-ident "+x" "1" "5"))
         (?place-pose (create-pose (list "map" (list 4.1 4 0.75) (list 0 0 0 1))))
         (?object-height 0.195d0)
         )
...
(defun test ()
  ...
  (urdf-proj:with-simulated-robot
    (let*
        ((object-ident (call-knowledge2 "has_urdf_name" :param-list "table_kitchen_shelves:table:table_front_edge_center" :result 'object))
         (pose (call-knowledge "object_rel_pose" :param-list `(,object-ident "perceive" (list "direction" "-x") result) :result 'result)))
