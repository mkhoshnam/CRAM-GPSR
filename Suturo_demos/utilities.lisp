...
(defun call-knowledge-object-rel-pose (object type direction)
  "Receives query `query', list of strings `param-list' and symbol `result' . Returns the query result for the chosen parameters. Used for querys that receive the 'result' as last parameter in prolog."
  (let* ((raw-response (with-safe-prolog
                         (json-prolog:prolog-simple-1
                          (concatenate 'string
                  "object_rel_pose('" object "', '" type "', [direction('" direction "')], POSESTAMPED)")))))
    ;; (roslisp:ros-info (knowledge-object-client)
    ;;                   "Goal pose for object ~a is ~a."
    ;;                   object-name
    ;;                   raw-response)
    (cond
      ...
      (t (fix-prolog-string (cdr (assoc (match-prolog-symbol 'PoseStamped) (cut:lazy-car raw-response))))))))

(defun call-knowledge-object-place-pose (object direction index maxindex)
  "Receives query `query', list of strings `param-list' and symbol `result' . Returns the query result for the chosen parameters. Used for querys that receive the 'result' as last parameter in prolog."
  (let* ((raw-response (with-safe-prolog
                         (json-prolog:prolog-simple-1
                          (concatenate 'string
                  "object_rel_pose('" object "', 'place', [direction('" direction "'), index(" index "), maxindex(" maxindex ")], POSESTAMPED)")))))
    ;; (roslisp:ros-info (knowledge-object-client)
    ;;                   "Goal pose for object ~a is ~a."
    ;;                   object-name
    ;;                   raw-response)
